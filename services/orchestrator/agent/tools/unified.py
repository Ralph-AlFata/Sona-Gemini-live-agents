"""Unified tool wrappers that consolidate 14 tools into 4 plus one batch tool.
...
# ---------------------------------------------------------------------------
# 5. canvas_actions — single agent-facing batch dispatcher
# ---------------------------------------------------------------------------
...
async def canvas_actions(
    actions: list[dict],
    stop_on_error: bool = True,
    tool_context: ToolContext | None = None,
) -> dict:
    """Execute a list of canvas actions sequentially through one tool call.

    Each item in `actions` must include:
    - `tool`: one of "draw", "edit_canvas", "graph", "highlight"
    - the remaining fields required by that underlying tool

    Actions execute in order. If `stop_on_error` is true (default), execution
    stops on the first exception or first action result whose status is
    `"error"`.
    """
    if not actions:
        raise ValueError("canvas_actions requires a non-empty 'actions' list")
    ...
    for index, raw_action in enumerate(actions):
        if isinstance(raw_action, str):
            try:
                ...
                raise ValueError(f"actions[{index}] must be an object") from exc
        ...
        if not isinstance(tool_name, str) or not tool_name:
            raise ValueError(f"actions[{index}] must include a non-empty 'tool' field")

        handler = _ACTION_HANDLERS.get(tool_name)
        if handler is None:
            raise ValueError(
                f"actions[{index}] has unsupported tool '{tool_name}'. "
                f"Supported tools: {sorted(_ACTION_HANDLERS)}"
            )

        try:
            result = await handler(tool_context=tool_context, **action)
        ...
        created_element_ids.extend(result.get("created_element_ids", []))

        if result.get("status") == "error":
            failed_action_index = index
            if stop_on_error:
                stopped_early = index < len(actions) - 1
                break
    ...
    return {
        "status": status,
        "operation": "canvas_actions",
        "applied_count": sum(int(result.get("applied_count", 0)) for result in results),
        "created_element_ids": created_element_ids,
        "failed_operations": [
            failed_operation
            for result in results
            for failed_operation in result.get("failed_operations", [])
        ],
        "results": results,
